...
async function deleteBorrow(id: any): Promise<void> {
    const response = await fetch(`http://localhost:5181/api/BorrowDetails/${id}`, {
      method: 'DELETE'
    });
    if (!response.ok) {
...
var tempBookStore:BookDetails;
async function conform_purchase(){
    let book_count=document.getElementById("book_count")as HTMLInputElement;

    let books=await fetchBookDetails();
    if(book_count.value != ""){
        books.forEach(book=>{
        ...
        if(Number(tempBookStore.bookCount) >= Number(book_count.value)){
                let newBorrow:BorrowDetails={
                    borrowID: undefined,
                    bookID: tempBookStore.bookID,
                    userID: tempUserStore.userID,
                    borrowDate: new Date().toString(),
                    borrowBookCount:Number(book_count.value),
                    status: "Borrowed",
                    paidFineAmount: 0
                }
                ...
                let changebookdetail:BookDetails={
                    bookID: tempBookStore.bookID,
                    bookName: tempBookStore.bookName,
                    authorName: tempBookStore.authorName,
                    bookCount: Number(tempBookStore.bookCount)-Number(book_count.value)
                }
                updateBookDetails(tempBookStore.bookID,changebookdetail);
...
async function returnPage(){
    ...
    let return_table=document.getElementById("return_table") as HTMLTableElement;

    let history=await fetchBorrowDetails();

    return_table.innerHTML="";

    return_table.innerHTML=`<tr>
                                <td>${"BookID"}</td>
                                <td>${"UserID"}</td>
                                <td>${"BorrowDate"}</td>
                                <td>${"BorrowBookCount"}</td>
    ...
    history.forEach(his=>{
        if(his.userID==tempUserStore.userID && his.status=="Borrowed"){
                return_table.innerHTML+=`<tr>
                                            <td>${his.bookID}</td>
                                            <td>${his.userID}</td>
                                            <td>${his.borrowDate}</td>
                                            ...
                                            <td>${his.status}</td>
                                            <td>${his.paidFineAmount}</td>
                                            <td><button onclick="conform_return(${his.bookID})">return</button></td>
                                        </tr>`
        }
    })
}
...
async function conform_return(id:any){

    let history=await fetchBorrowDetails();
    let books=await fetchBookDetails();
    let return_book:BorrowDetails;
    history.forEach(his=>{
        if(his.bookID==id && his.status=="Borrowed"){

            return_book={
                borrowID:undefined,
                bookID: his.bookID,
                userID: his.userID,
                borrowDate: his.borrowDate,
                borrowBookCount: his.borrowBookCount,
                status: "Returned",
                paidFineAmount: his.paidFineAmount
            }

            addBorrowDetails(return_book);
        }})
    
    
        books.forEach(book=>{
            if(return_book.bookID==book.bookID){
                let updateBook1:BookDetails={
                    bookID: return_book.bookID,
                    bookName: book.bookName,
                    authorName: book.authorName,
                    bookCount: Number(book.bookCount) +Number(return_book.borrowBookCount)
                }
                updateBookDetails(return_book.bookID,updateBook1);
                deleteBorrow(return_book.bookID);
                returnPage();
